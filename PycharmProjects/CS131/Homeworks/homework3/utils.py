...
def plot_matches(ax, image1, image2, keypoints1, keypoints2, matches,
    ...
    for i in range(matches.shape[0]):
        ...
        ax.plot((keypoints1[idx1, 1], keypoints2[idx2, 1] + offset[1]),(keypoints1[idx1, 0], keypoints2[idx2, 0]),'-', color=color)
...
def get_output_space(img_ref, imgs, transforms):
        ...
        for i in range(len(imgs)):
            ...
            H = transforms[i]
            corners = np.array([[0, 0], [r, 0], [0, c], [r, c]])
            warped_corners = corners.dot(H[:2, :2]) + H[2, :2]      #省去了将corner的每一行末尾添加上一个1
