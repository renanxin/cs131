...
from Homeworks.homework3.utils import pad, unpad
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(N):
        dist=dists[i,:]
        if np.min(dist) / (np.partition(dist,2)[1]) <= threshold:       #对于函数partition则是得到数组中是以第二个大的数为中，小于它的放前面，大于或者等于放在后面，这样一来就得到第二大的数字
...
def fit_affine_matrix(p1, p2):
    ...
    ### YOUR CODE HERE
    H = np.linalg.lstsq(p2, p1, rcond=None)[0]  #对于函数lstsq是得到一个方程a*x=b的x的最小二乘法结果，第二个返回结果为b-a*x
    ### END YOUR CODE
