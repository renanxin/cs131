...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    ### YOUR CODE HERE
    for i in range(0, response_map.shape[
        0]):  # 考虑到H和W不一定是winH和winW的整数倍，是否可以采取这样的措施：循环只到倒数第二行/列，最后一行/列单独处理，最后剩下多大就跟多大的face_feature做相关，最后记得均衡一下（得到的响应值/最后一块的大小*正常块的大小）
        for j in range(0, response_map.shape[1]):
            ...
            response_map[i, j] = np.sum(
                base_score * hogFeature)  # / (sum_hog * np.sum(hogFeature)) #之前还在想用不用去除能量的影响，以为一张白纸跟人脸做相关会是最大，后来发现不对，做相关的不是原像素而是hog特征，而白纸的hog特征是全零，因此不会是最大。而且去除能量的效果并不好，得到了错误的识别结果，综上，不需要去除能量的影响
            # 这样得到的位置似乎更精确一点
            # if score>max_score:
            #     max_score=score
    ...
    maxr, maxc = int(max_index / response_map.shape[1] - np.ceil(winH // 2 / stepSize)), int(
        max_index % response_map.shape[1] - np.ceil(
            winW // 2 / stepSize))  # 因为直接argmax出来的下标是与填充后的图像相对应的下标，而输出要的是对应与原图片，所以需要做一下校正
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    r,c=np.unravel_index(np.argmax(heatmap_face),heatmap_face.shape)        #返回一个flatten矩阵中一个元素序列在第二个参数大小的矩阵中各个坐标上的位置
    ### END YOUR CODE
