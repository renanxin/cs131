...
def kmeans(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ### YOUR CODE HERE
        for i in range(N):
            assignments[i]=np.argmin(np.sum((centers-features[i])**2,axis=1))
        temp=centers.copy()
        for j in range(k):
...
def kmeans_fast(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ### YOUR CODE HERE
        f_tmp=np.repeat(features,k,axis=0)
        c_tmp=np.tile(centers,(N,1))
        assignments=np.argmin(np.sum((f_tmp-c_tmp)**2,axis=1).reshape(N,k),axis=1)      #对于下表axis，则是沿着axis变化的那个方向进行操作的
        temp=centers.copy()
        for j in range(k):
            centers[j] = np.mean(features[assignments == j], axis=0)
        if np.allclose(temp,centers):
...
def hierarchical_clustering(features, k):
    ...
    while n_clusters > k:
        ...
        matrixDistances=np.where(matrixDistances!=0,matrixDistances,1e10)
        minValue=np.argmin(matrixDistances)
        min_i=minValue//n_clusters
        min_j=minValue-min_i*n_clusters
        if min_i>min_j:
            min_i,min_j=min_j,min_i
        assignments[assignments==min_j]=min_i
        assignments[assignments>min_j]-=1
...
def color_position_features(img):
    ...
    ### YOUR CODE HERE
    location=np.dstack(np.mgrid[0:H,0:W]).reshape(H*W,2)
...
img=io.imread('train.jpg',as_gray=True)
a,b,c=np.linalg.svd(img)
n=100
new_img=np.dot(np.dot(a[:,:n],np.diag(b[:n])),c[:n,:])
