...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        convariance=np.dot(X.T,X)/(N-1)     #由于X为0均值
        e_vals,e_vecs=np.linalg.eig(convariance)        #这步非常耗时
        idx=np.argsort(-e_vals)     #由于结果要的是从大到小，而argsort默认为从小到大
        e_vals=e_vals[idx]
        e_vecs=e_vecs[:,idx]    #特征向量是存储在列向量中
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        #     2. add the mean  we substracted in `transform`
        X=self.W_pca[:,:n_components].dot(X_proj.T)
        X=X.T+self.mean
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        e_vals,e_vecs=scipy.linalg.eig(scipy.linalg.inv(scatter_within).dot(scatter_between))
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            S_i=X_i_centered.T.dot(X_i_centered)
            scatter_within=scatter_within+S_i
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            B_i=X_i.shape[0]*(X_i_mean.dot(X_i_mean))
            scatter_between=scatter_between+B_i
            # END YOUR CODE
