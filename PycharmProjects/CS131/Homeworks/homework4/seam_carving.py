...
def energy_function(image):
    ...
    gradients=np.gradient(gray)     #返回值为一个数组，其中包含了每一维的梯度
...
def compute_cost(image, energy, axis=1):
    ...
    for i in range(1,H):
        center=cost[i-1,:]
        left=np.hstack(([10000000.],cost[i-1,0:-1]))
        right=np.hstack((cost[i-1,1:],[10000000.]))
        temp=np.vstack((left,center,right))
        cost[i,:]=energy[i,:]+np.min(temp,axis=0)
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(k):
        ...
        assert np.all(seams[np.arange(H), indices[np.arange(H), seam]] == 0),"we are overwriting seams"
...
def reduce_fast(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    while out.shape[1] > size:
        ...
        if i <=1:
            ...
            energy = np.c_[energy[:, 0: i-1], efunc(out[:, i-2: j+3])[:,i-1 : -1], energy[:, j+2:]]
    ### END YOUR CODE
...
def remove_object(image, mask):
    ...
    H,W,C=image.shape
    ### YOUR CODE HERE
    assert mask.shape==(out.shape[0],out.shape[1])

    mask = mask.astype('int')  # 把bool变成int

    len = np.sum(np.amax(mask, axis=1))  # 计算mask非0的长度
    mask = (mask == 0)  ##应该适当把mask扩大
    ...
    out = reduce(out, W - int(len * 1.65), efunc=lambda x: (energy), cfunc=lambda x, y: (cost, paths))
    plt.subplot(2,2,4)
    plt.imshow(out)
    out = enlarge(out, W)
